...
def survey(request):
    if request.method == 'POST':
        ...
        if old_form.is_valid():
            ...
            selected_choice = Choice.objects.filter(question = Question.objects.get(id = request.POST['question'])).get(choice_text = selected_option)
            selected_choice.votes += 1
            selected_choice.save()
            last_data_point = DataPoint.objects.create(
                    loggist = request.user,
                    survey_instance = SurveyInstance.objects.get(id = request.POST['survey_instance']),
        ...
        if loggist.has_uncompleted_survey == True:
            loggists_last_data_point = DataPoint.objects.filter(loggist = request.user).order_by('-time_entered')[:1][0]
            print(loggists_last_data_point)
            if loggists_last_data_point.question.id == Question.objects.count():
                local_question = Question.objects.get(id=1)
                local_survey_instance = SurveyInstance.objects.create()
            else:
                local_question = Question.objects.get(id = loggists_last_data_point.question.id + 1)
